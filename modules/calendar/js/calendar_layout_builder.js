function display_events(calendar, event_data){
	...
	for(var i = 0; i < num_valid_events; i++){
		...
		var current_event = calendar.event_data.events[event_index];
...
function insert_moons(data){
	...
	for(moon_index = 0; moon_index < calendar.moons.length; moon_index++){

		var moon = calendar.moons[moon_index];
...
function update_moon_colors(){
	...
	for(var index = 0; index < calendar.moons.length; index++){

		color = calendar.moons[index].color ? calendar.moons[index].color : '#ffffff';
...
var eras = {
	...
	set_current_era: function(index){
		...
		if(this.era != index){
			...
			this.internal_class.innerHTML = " - " + this.current_eras[this.era].data.name;
		}
...
var calendar_layouts = {
	...
	add_year_day_number: function(){
		if(calendar.settings.add_year_day_number){
	...
	add_month_number: function(){
		if(calendar.settings.add_month_number){
	...
	insert_empty_calendar: function(data){
		...
		this.layout = calendar_layouts[calendar.settings.layout];
	...
	insert_calendar: function(data){
		...
		for(var i = 0; i < Object.keys(this.timespans).length; i++){
			...
			var timespan = this.timespans[Object.keys(this.timespans)[i]];
			...
			this.layout.insert_intercalary_day(filtered_leap_days_afterend, timespan.length);
	...
	grid: {
		...
		insert_timespan: function(timespan){
			...
			if(timespan.type === 'month'){
				...
				this.insert_intercalary_day(timespan, filtered_leap_days_beforestart, timespan.length);
				...
				calendar_layouts.html.push("<div class='timespan_row_container'>");
					...
					calendar_layouts.html.push("<div class='timespan_row'>");
						...
						for(timespan_day = 1; timespan_day <= timespan.length; timespan_day++, calendar_layouts.year_data.year_day++, calendar_layouts.year_data.epoch++){
							...
							this.insert_intercalary_day(timespan, filtered_features, timespan.length);
				...
				calendar_layouts.html.push("<div class='timespan_container grid'>");
					...
					calendar_layouts.html.push("<div class='timespan_name'>");
						calendar_layouts.html.push(timespan.name + (calendar.settings.add_month_number ? " - Month " + (timespan.index+1) : ""));
					calendar_layouts.html.push("</div>");
		...
		insert_intercalary_day: function(timespan, filtered_features, length){
		...
		insert_intercalary_day: function(timespan, filtered_features, length){
			...
			if(filtered_features.length > 0){
				...
				for(index = 0; index < filtered_features.length; index++){
					...
					this.insert_day(calendar_layouts.year_data.epoch, '', "timespan_day timespan_intercalary", feature.name);
		...
		insert_day: function(epoch, day_num, day_class, title, intercalary){
			...
			calendar_layouts.html.push(`<div class='${day_class}' epoch='${epoch}'>`);
				...
				/*calendar_layouts.html.push("<div class='day_row weather'>");
					...
					temp = "";
					switch(calendar.climate.global_settings.temp_sys){
						case "metric":
					...
					if(weather.wind_velocity_m !== undefined && weather.wind_velocity_i !== undefined){
						wind += " (";
						switch(calendar.climate.global_settings.wind_sys){
							case "metric":

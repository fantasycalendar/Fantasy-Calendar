...
@section("content")
	...
	<div class="container p-4" x-data="agreement_form()">
		...
		<h1>{{ $title }}</h1>
		<p><i>{{ $date }}</i></p>
		<div class="scroll_box mb-4 border p-3 rounded">{!! Markdown::convertToHtml($markdown); !!}</div>
		...
		<div class="form-check border rounded p-2 mb-3">
			...
			<label class="form-check-label" for="acknowledgement">By clicking <strong>Agree and Continue</strong>, I hereby agree and constent to the <a target="_blank" href="{{ route('terms-of-service') }}">Terms of Service</a>, and the <a target="_blank" href="{{ route('privacy-policy') }}">GDPR Privacy Policy</a></label>
		</div>

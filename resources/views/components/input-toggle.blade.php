@php
    $uid = $attributes->get('id') ?? \Illuminate\Support\Str::slug($attributes->get('name') ?? '') ?: \Illuminate\Support\Str::random(6);
...
<div {{ $mainAttributes }} x-data="{ value: {{ $attributes->get('value', 'false') }} }" x-modelable="value">
    ...
    <div class="flex-grow flex flex-col">
        <span class="text-sm font-medium text-gray-900 dark:text-gray-300" id="{{ $uid }}-label">{{ $attributes->get('label') }}</span>

        @if($attributes->has('description'))
            <div class="text-sm text-gray-500 dark:text-gray-400" id="{{ $uid }}-description">{{ $attributes->get('description') }}</div>
        @endif

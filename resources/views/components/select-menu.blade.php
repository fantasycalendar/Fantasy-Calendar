<div x-data='{
    ...
    >
    <label id="listbox-label" class="block font-medium text-gray-700 dark:text-gray-400">
        {{ $slot }}
    </label>

    <div class="mt-1 relative">
        <button @click="isOpen =! isOpen" type="button" {{ $attributes->merge(['class' => 'bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-400 relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm']) }} aria-haspopup="listbox" aria-expanded="true" aria-labelledby="listbox-label">
        ...
        <ul class="absolute z-10 mt-1 w-full bg-white dark:bg-gray-700 shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm" tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-option-3"
            ...
            <template x-for="option in options">
                <li class="text-gray-900 dark:text-gray-300 cursor-default select-none relative py-2 pl-3 pr-9 hover:text-white dark:hover:text-white hover:bg-primary-600 dark:hover:bg-primary-800"
                    :class="{ 'text-white dark:text-white bg-primary-600': chosen.value == option.value }"
                    role="option"
                    @click="select(option)"
                >

    ...
    show: function(day_element, epoch_data){
        ...
        for(var index in keys){
            ...
            if(data !== undefined){
                ...
                if(key == "era"){
                    if(data > -1){
                        html.push(window.calendar.static_data.eras[data].name);
                    }
                }else if(key.indexOf('moon') > -1){
                    for(var moon_index in data){
                        var moon = window.calendar.static_data.moons[moon_index];
                        if(key == "moon_phase"){
                    ...
                    for(var cycle_index in data){
                        ...
                        html.push(`${window.calendar.static_data.cycles.data[cycle_index].names[cycle_key]}<br>`);

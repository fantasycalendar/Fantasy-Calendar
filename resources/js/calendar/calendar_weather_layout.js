	...
	tooltip: {
		...
		show: function(icon){
			...
			if(calendar_weather.processed_weather && !icon.hasClass('noweather')){
				...
				var temp = "";
				if(!static_data.settings.hide_weather_temp || window.Perms.player_at_least('co-owner')){
				...
				this.weather_temp.toggleClass('newline', (temp_sys == 'both_i' || temp_sys == 'both_m') && (!static_data.settings.hide_weather_temp || window.Perms.player_at_least('co-owner')));
				...
				if(wind_sys == 'both'){
					...
					if(!static_data.settings.hide_wind_velocity || window.Perms.player_at_least('co-owner')){
				...
				this.weather_temp.each(function(){
					$(this).html(temp);
				}).parent().toggleClass('hidden', static_data.settings.hide_weather_temp !== undefined && static_data.settings.hide_weather_temp && !window.Perms.player_at_least('co-owner'));

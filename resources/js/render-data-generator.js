    ...
    _create_render_data: function(processed_data){
        ...
        for(let index = 0; index < timespans_to_build.length; index++){
            ...
            if(filtered_leap_days_beforestart.length > 0){
                ...
                let timespan_data = {
                    "title": "",
                    "id": [index, timespan.name, timespan.index, epoch, dynamic_data.year, filtered_leap_days_beforestart.length].join('.'),
                    "show_title": false,
            ...
            let timespan_data = {
                ...
                "id": [index, timespan.name, timespan.index, epoch, dynamic_data.year].join('.'),
                "show_title": true,
            ...
            if(Perms.player_at_least('co-owner') || !static_data.settings.only_reveal_today || (static_data.settings.only_reveal_today && epoch > dynamic_data.epoch)){
                ...
                if(filtered_leap_days_end.length > 0){
                    ...
                    let timespan_data = {
                        "title": "",
                        "id": [index, timespan.name, timespan.index, epoch, dynamic_data.year, filtered_leap_days_end.length].join('.'),
                        "show_title": false,
                        "short_weekdays": timespan.truncated_week,
                        "weekdays": static_data.year_data.global_week,
                        "show_weekdays": false,
